// REQUIRES: dxil-1-10
// RUN: %dxc -HV 202x -I %hlsl_headers -T lib_6_10 -enable-16bit-types -fcgl %s | FileCheck %s
// RUN: %dxc -HV 202x -I %hlsl_headers -T lib_6_10 -enable-16bit-types %s | FileCheck %s --check-prefix=CHECKVAL

#include <dx/linalg.h>
using namespace dx::linalg;

// CHECK: %dx.types.LinAlgMatrixC4M4N5U1S2 = type { i8* }
// CHECK: %dx.types.LinAlgMatrixC2M3N3U0S1 = type { i8* }
...
// CHECK: define internal void @"\01?f2@@YAX$linalg_matrixC2M3N3U0S1@@Z"(%dx.types.LinAlgMatrixC2M3N3U0S1 %mat2.coerce)
// CHECK: %{{.*}} = alloca %dx.types.LinAlgMatrixC2M3N3U0S1, align 4
void f2(__builtin_LinAlgMatrix [[__LinAlgMatrix_Attributes(ComponentType::I16, 3, 3, MatrixUse::A, MatrixScope::Wave)]] mat2) {
}
...
// CHECK: ![[T1:.*]] = !{%dx.types.LinAlgMatrixC2M3N3U0S1 undef, i32 2, i32 3, i32 3, i32 0, i32 1}
